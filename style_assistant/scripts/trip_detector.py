...
def extract_destination_from_ticket(image_bytes: bytes) -> str:
    ...
    for attempt in range(5):
        try:
            ...
            if "429" in error_msg:
                if attempt < 4:
                    logging.warning(f"[TripDetector] Rate limit hit. Waiting 15s... (Attempt {attempt+1}/5)")
                    time.sleep(15)
                    continue
                else:
                    logging.error("[TripDetector] Groq Vision pipeline failed due to repeated rate limits.")
        ...
        for attempt in range(5):
            try:
                ...
                if "429" in str(e) and attempt < 4:
                    logging.warning(f"[TripDetector] LLM rate limit hit. Waiting 10s... (Attempt {attempt+1}/5)")
                    time.sleep(10)
                    continue
                else:
                    raise e
