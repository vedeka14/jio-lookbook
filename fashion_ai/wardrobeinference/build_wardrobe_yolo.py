...
def build_wardrobe(silent=False, source_dir=None, max_images=200):
    def _print(*args, **kwargs):
        if not silent:
            print(*args, **kwargs)

    # Determine which folder to scan
    scan_dir = Path(source_dir) if source_dir else PHOTOS_DIR

    _print("=" * 60)
    _print(f"Running YOLO Wardrobe Inference on : {DEVICE.upper()}")
    if DEVICE == "cuda":
        _print(f"GPU : {torch.cuda.get_device_name(0)}")
    _print(f"Source folder: {scan_dir}")
    _print("=" * 60)
    ...
    # ==================================================
    # Read Images (cap at max_images for CPU speed)
    # ==================================================
    ...
    _print("\n" + "=" * 60)
    _print(f"Found {len(all_image_paths)} image(s) in {scan_dir.name}, processing {len(image_paths)}")
    _print("=" * 60)
